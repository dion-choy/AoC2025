let input =
  List.map
    ...
    (String.split_on_char ','
       (String.trim
          (In_channel.with_open_text "./day2.txt" In_channel.input_all)))

let part1 input =
  let check_num num_str =
    if String.length num_str mod 2 = 0 then
      if
        String.sub num_str 0 (String.length num_str / 2)
        = String.sub num_str
            (String.length num_str / 2)
            (String.length num_str / 2)
  ...
  let rec range num target =
    ...
    else check_num num + range (string_of_int (int_of_string num + 1)) target
  in
...
let part2 input =
  let rec all_equal prev lst =
    ...
    | [] -> true
  in
  let rec split_str num str =
    if String.length str = 0 then []
    else
      String.sub str 0 num
      :: split_str num (String.sub str num (String.length str - num))
  in
  let rec repeats num num_str =
    if num = String.length num_str then 0
    else if String.length num_str mod num = 0 then
  ...
  let rec range num target =
    if int_of_string num > int_of_string target then 0
    else repeats 1 num + range (string_of_int (int_of_string num + 1)) target
  in
  List.fold_left (fun acc (min, max) -> acc + range min max) 0 input
