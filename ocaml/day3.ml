let input =
  ...
  List.map
    ...
    (String.split_on_char '\n'
       (String.trim
          (In_channel.with_open_text "./day3.txt" In_channel.input_all)))
...
let part1 input =
  ...
  let process lst =
    ...
    + find_max
        (List.drop
           (find_index_max (List.take (List.length lst - 1) lst) + 1)
           lst)
  in
...
let part2 input =
  ...
  let rec process cnt row =
    ...
    if cnt >= 0 then
      process (cnt - 1) (List.drop (find_index_max sub_row + 1) row)
